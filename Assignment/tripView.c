...
int gettime(int time, int tt)  //This function adds the time of a station and transfer time.
{
    int hh = time / 100;
    int mm = time % 100;
...
void PrintBFSPath(Graph g, int from_sta_no, int y, timetable** timetables) //print out Path //Time complexity is O(m*s)
{
    {
        int a = y;
...
bool findPathBFS(Graph g, int src, int dest)   //Time complexity is O(m*s)
{
    int v;
...
int stage2(char fromname[17], char toname[17], char arrivaltime[5], timetable** timetables) //Time complexity (O(m*s)^2)
{
    int i, j, k, l;
...
int stage1(char fromname[17], char toname[17], char arrivaltime[5], timetable** timetables)  //Time complexity is O(m*s)
{
...
int main(void) {
    ...
    printf("From: ");
    memset(fromname, '\0', sizeof(fromname));  //Ensure all 16 characters are initialized to 0 before scanning a new string. 
